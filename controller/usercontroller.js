...
const {sendOtpEmail, sendMfaOtpEmail, sendLoginNotificationEmail} = require("../utils/emailserivce");
...
// Shared final step of login: records activity, issues the JWT + session token,
// sends the login-notification email and returns the token. Used by both the
// normal login path and the MFA (verify-login-otp) path.
const completeLogin = async (user, req, res) => {
  ...
  // never leak the hidden fields back to the client
...
const authController = {
  ...
  login: async (req, res) => {
    ...
    //  MFA ENABLED → send an email OTP and stop here.
    //  The client must then call POST /verify-login-otp to receive the token.
    if (user.mfaEnabled) {
      const { otp, otpExpiresAt } = await generateOtp();
      ...
      await sendMfaOtpEmail(user.email, otp, user.fullName);
    ...
    //  MFA DISABLED → normal login, issue the token immediately.
    return completeLogin(user, req, res);
  },
  // Step 2 of MFA login: verify the emailed OTP and issue the token.
  verifyLoginOtp: async (req, res) => {
    ...
    const user = await userService.getUserByEmailWithMfaOtp(email.toLowerCase());
    ...
    if (!user.mfaEnabled) {
      return res.status(400).json({ status: "failed", message: "MFA is not enabled for this account" });
    }

    if (!user.mfaOtp || !user.mfaOtpExpiresAt) {
      return res.status(400).json({ status: "failed", message: "No pending login code. Please log in again." });
    }

    if (new Date() > user.mfaOtpExpiresAt) {
      return res.status(401).json({ status: "failed", message: "Login code has expired. Please log in again." });
    }

    if (user.mfaOtp !== otp) {
      return res.status(401).json({ status: "failed", message: "Invalid login code" });
    }

    // consume the OTP so it can't be reused
    await userService.updateUser(user._id, {
      $unset: { mfaOtp: "", mfaOtpExpiresAt: "" },
    });

    return completeLogin(user, req, res);
  },

  // Turn MFA on for the logged-in user.
  enableMfa: async (req, res) => {
    ...
    if (user.mfaEnabled) {
      return successResponse(res, { mfaEnabled: true }, "MFA is already enabled", STATUSCODES.SUCCESS);
    }
    ...
    return successResponse(
      res,
      { mfaEnabled: true },
      "Two-factor authentication enabled. You'll receive a login code by email each time you sign in.",
      STATUSCODES.SUCCESS
    );
  },

  // Turn MFA off. Requires the account password to prevent a hijacked
  // session from silently disabling protection.
  disableMfa: async (req, res) => {
    const userId = req.user.userId;
    const { password } = req.body;

    if (!password) {
      return res.status(400).json({ status: "failed", message: "Password is required to disable MFA" });
    }
    ...
    return successResponse(res, { mfaEnabled: false }, "Two-factor authentication disabled.", STATUSCODES.SUCCESS);
  },

  // Convenience: let the client read the current MFA state.
  getMfaStatus: async (req, res) => {
    ...
    return successResponse(res, { mfaEnabled: !!user.mfaEnabled }, "MFA status retrieved", STATUSCODES.SUCCESS);
  },
