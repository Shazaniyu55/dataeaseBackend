...
/**
 * Send a login OTP for two-factor authentication (MFA)
 * @param email Recipient's email address
 * @param otp   One-time login code
 * @param fullName Optional recipient name
 */
const sendMfaOtpEmail = async (email, otp, fullName) => {
  const subject = `${APP_NAME} login code: ${otp}`;
  const name = capitalizeEachWord(fullName) || "there";
  const html = `
  <!DOCTYPE html>
  <html lang="en">
    <head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /></head>
    <body style="font-family: Arial, sans-serif; background-color:#f7f7f7; margin:0; padding:24px;">
      <div style="max-width:480px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px; text-align:center;">
        ...
        <p style="color:#555; margin:0 0 24px;">Hi ${name}, use the code below to finish signing in to ${APP_NAME}.</p>
        <div style="font-size:32px; font-weight:700; letter-spacing:6px; color:#111; background:#f2f2f2; border-radius:8px; padding:16px 0;">${otp}</div>
        <p style="color:#888; font-size:13px; margin:24px 0 0;">This code expires in 5 minutes. If you didn't try to log in, change your password immediately.</p>
      </div>
    </body>
  </html>`;
  await sendEmail(email, subject, html);
...
module.exports = {sendOtpEmail, sendMfaOtpEmail, sendForgotPasswordEmail, sendLoginNotificationEmail, SendResetOTP, SendPromotionalEmail};
